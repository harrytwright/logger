...
const tryRequire = (path) => {
  try { return require(path) } catch (_) { emitMyWarning(`Failed to find '${path}'`, `ERR_MISS_${path}`); return null }
}
...
if (isNextJS) {
  emitMyWarning('For using next.js please use \'@harrytwright/logger/dist/next\'', 'ERR_NEXT_JS')
}
...
module.exports = new Log(name, isProduction ? 'info' : 'silly', initialRedactions)
